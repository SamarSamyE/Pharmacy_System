...
class AreasDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        <a class="btn btn-success" id="option_a1" href="#"> edit </a>
        <form method="post" class="delete_item"  id="option_a3" action="#">
            @csrf
            @method("DELETE")
            <button type="submit" class="btn btn-danger" onclick="modalShow(event)" id="#" data-bs-toggle="modal" data-bs-target="#exampleModal">delete</button>
        </form>
    </div>')
       ->addColumn('Country', function (Area $area) {
        // $area = Area::all()->first();
        //dd($area->country->name);
